...
.synchronize:
        ...
        mov     [rsi + 8 * rbx], rax    ; set the value[n] to the value to exchange with core 'm',
                                        ; no need to atomically set the value

        mov     rax, r8                 ; copy the value 'm'
        lea     rdi, [rel spin_lock]    ; get the address of spin locks
        xchg    [rdi + 8 * rbx], rax    ; set the value of spin_lock[n] to 'm' meaning that core 'n'
...
.busy_wait:
        cmp     qword [rdx], rbx        ; check if core 'm' is waiting for core 'n'
        jnz     .busy_wait
        ...
        lea     rdx, [rdi + 8 * rbx]    ; get the address of spin_lock[n]
.wait:
